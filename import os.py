...
results_csv = "ExpirementCombined9_Complete_Analysis.csv"
plots_dir = "results/ExpirementCombined9_Analysis"
...
def extract_path_info(file_path):
    ...
    for i, part in enumerate(parts):
        ...
        elif "input" in part and "output" in part:
...
def parse_log_file(file_path):
    ...
    with open(file_path, "r", encoding='utf-8', errors='ignore') as file:
        ...
        for i, line in enumerate(lines):
            ...
            # Extract HP Configuration Name - FIXED
            if "Name:" in line:
                hp_match = re.search(r'Name:\s*(.+)', line)
                if hp_match:
                    hp_name = hp_match.group(1).strip()

            # Extract Optimizer Type - FIXED
            ...
            # Extract Skip Percentage - FIXED to handle both formats
            if "Skip Percentage" in line and ":" in line:
                skip_match = re.search(
                    r'Skip Percentage[^:]*:\s*([\d\.]+)%?', line)
                if skip_match:
                    skip_val = float(skip_match.group(1))
                    # If value is > 1, it's already a percentage
                    skip_percentage = skip_val if skip_val <= 1 else skip_val
            ...
            # Extract TEACHER architecture - FIXED
            if "Teacher:" in line and ("-" in line or "n" in line):
                teacher_match = re.search(r'Teacher:\s*(.+)', line)
                if teacher_match:
                    teacher_architecture = teacher_match.group(1).strip()

            # Extract STUDENT architecture - FIXED
            if "Student:" in line and ("-" in line or "n" in line):
                student_match = re.search(r'Student:\s*(.+)', line)
                if student_match:
                    student_architecture = student_match.group(1).strip()

            # Also check for "Student Config Used:" line
            if "Student Config Used:" in line:
                config_match = re.search(r'Student Config Used:\s*(.+)', line)
                if config_match:
                    student_architecture = config_match.group(1).strip()

            # Parse layer config from STUDENT architecture
            if student_architecture and not layer_config:
                # Parse architecture like "9n-40n-4o"
                arch_parts = student_architecture.split('-')
                for part in arch_parts:
                    match = re.search(r'(\d+)[no]', part)
                    if match:
                        nodes = int(match.group(1))
                        layer_config.append(nodes)
                        # Don't count input/output in total_nodes
                        if 'n' in part and part != arch_parts[0]:
                            total_nodes += nodes
            ...
            if "Network Structure:" in line:
                for j in range(i + 1, min(i + 10, len(lines))):
                    ...
                    if not struct_line or "===" in struct_line or "Final" in struct_line:
                        break
                    layer_match = re.search(
                        r'(\w+(?: \w+)?):\s*(\d+)\s*nodes,\s*(\d+)\s*edges', struct_line)
                    if layer_match:
            ...
            if "Learning Rate" in line and ":" in line:
                lr_match = re.search(
                    r'Learning Rate[^:]*:\s*([\d\.E-]+)', line)
                if lr_match:
                    learning_rate = float(lr_match.group(1))
            elif "Dropout Rate:" in line:
                dropout_match = re.search(r'Dropout Rate:\s*([\d\.]+)', line)
                if dropout_match:
            ...
            elif "Adam Beta" in line or "beta1=" in line:
            ...
            elif "RMSprop" in line or "decay=" in line:
            ...
            # Extract parameter counts - FIXED
    ...
    if layer_config:
        arch_description = "-".join([f"{n}{'n' if i < len(layer_config)-1 else 'o'}"
                                     for i, n in enumerate(layer_config)])
        depth = len(layer_config) - 2  # Exclude input and output layers
    else:
        arch_description = student_architecture if student_architecture else "Unknown"
        depth = None
    ...
    return {
        ...
        'architecture': student_architecture if student_architecture else arch_description,
    ...
    for root, _, files in os.walk(base):
        if log_file_name in files:
            ...
            best_epoch = data['val_accuracy'].index(
                best_val_acc) + 1 if best_val_acc else None
            ...
            # FIXED: Use the extracted architecture
            student_arch = data['architecture'] if data['architecture'] and data['architecture'] != "Unknown" else "NA"
            nodes = data['arch_description'] if data['arch_description'] else "NA"
            depth = data['depth'] if data['depth'] is not None else "NA"
            ...
            val_acc_display = f"{final_val_acc:.2f}" if final_val_acc is not None else "0.00"
            print(
                f"  ✓ Processed | HP: {hp_name} | Arch: {student_arch} | Params: {data['student_params']} | ValAcc: {val_acc_display}%")
